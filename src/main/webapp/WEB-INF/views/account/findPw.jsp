...
<div id="wrap">
	...
	<h2>비밀번호 찾기</h2>
	<div class="ment" style="margin-top: 10px">회원가입 시 입력하신 정보로 임시 비밀번호를 발급 받을 수 있습니다. 로그인 후 비밀번호를 꼭 변경해주세요.</div>

	<div class="container-wrap">
		<form name="workForm" action="<c:out value="${cPath}"/>/account/find-pw">
			<div>
				<label class="required" for="adminId">
					회원 아이디
				</label>
				<input type="text"		name="adminId"	 id="adminId"	required autocomplete="off" placeholder="ID를 입력해주세요.">
				<input type="button"	id="btnExistCheckAdminId"	value="확인"		class="">
			</div>
...
<script type="text/javascript">
	...
	const btnExistCheckAdminId = document.getElementById('btnExistCheckAdminId');
	...
	btnExistCheckAdminId.addEventListener('click', () => existCheckAdminId(adminIdInput.value, existCallback));
	function existCallback(isExist){
		if(isExist)	alert('확인되었습니다.');
		else alert('존재하지 않는 아이디입니다.');
	}

	// 필수값이 모두 입력되었을때, form submit 활성화
	const submitBtn = document.querySelector('input.submitBtn');
	handleSubmitBtnEnable(submitBtn, [adminIdInput, [phoneAuthNumInput, emailAuthNumInput]]);
</script>
<script type="text/javascript">
